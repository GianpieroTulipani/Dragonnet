...
def train_and_predict(
    ...
    t_trainval, t_test, y_trainval, y_test, x_trainval, x_test = train_test_split(t, y, x, test_size=val_split, random_state=seed)

    t_train, t_val, y_train, y_val, x_train, x_val = train_test_split(t_trainval, y_trainval, x_trainval, test_size=val_split, random_state=seed)
    ...
    for epoch in range(num_epochs):
        ...
        for x_batch, y_batch, t_batch in tqdm(train_loader, desc=f" Epoch {epoch+1}", leave=False):
            x_batch = x_batch.to(device)
            y_batch = y_batch.to(device)
            t_batch = t_batch.to(device)
        ...
        with torch.no_grad():
            for x_batch, y_batch, t_batch in tqdm(val_loader, desc=f"Epoch {epoch+1} Val", leave=False):
                x_batch = x_batch.to(device)
                y_batch = y_batch.to(device)
                t_batch = t_batch.to(device)
    ...
    print(f"Test regression MSE: {test_mse:.4f}")
